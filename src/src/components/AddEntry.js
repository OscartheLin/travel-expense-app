...
export default function AddEntry({ book, onSave, onCancel }) {
  ...
  const [form, setForm] = useState({
    ...
    method: '信用卡', card: '', payer: '', category: '餐飲', note: ''
  });
    ...
    <div className="add-form">
      ...
      <div className="form-section" style={{ marginTop: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
          {(book.people ? ['婷', ...book.people.split(/[,，、]/).map(s => s.trim()).filter(Boolean)] : ['婷', '珽']).map(person => (
            <button key={person}
              className={`payer-btn ${form.payer === person ? (person === '婷' || person === '美眉' ? 'active-t' : 'active-d') : ''}`}
